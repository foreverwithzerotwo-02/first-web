<template>
  <div class="user-manage-permission">
    <!-- 顶部区域 -->
    <div class="user-manage-permission__top-region">
      <!-- 标题 -->
      <h2 class="umr__tr__title">权限管理</h2>

      <!-- 搜索框和新增按钮 -->
      <div class="umr__tr__actions">
        <!-- 搜索框 -->
        <div class="umr__tr__search">
          <el-input v-model="searchKeyword" placeholder="搜索权限名称" clearable @input="handleSearch" @clear="handleSearch"
            class="umr__tr__search-input">
            <template #prefix>
              <el-icon>
        ...
        <!-- 新增权限按钮 -->
        <el-button type="primary" @click="openAddPermissionsDrawer" class="umr__tr__add-btn">
    ...
    <!-- 权限列表表格内容区域 -->
    <div class="user-manage-permission__main-table">
      <el-table :data="allPermissions" border style="width: 100%; height: 100%;" stripe v-loading="loading"
        ...
        <el-table-column prop="code" label="权限代码" min-width="200" sortable />
        <el-table-column label="操作" width="180" align="center" fixed="right">
          <template #default="scope">
            <el-button link type="primary" size="small" @click="openEditPermissionsDrawer(scope.row)">
              编辑
            </el-button>
            <el-button link type="danger" size="small" @click="handleDeletePermissions(scope.row)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <!-- 新增或编辑权限抽屉 -->
    <drawer v-model="isOpenPermissionsDrawer" :width="drawerWidth" placement="right" :title="drawerTitle">
      <my-form ref="permissionsFormRef" :form="permissionsFormData" :rules="permissionsFormRules" label-align="left">
        <my-form-item prop="name" label="权限名称" required>
          <el-input v-model="permissionsFormData.name" placeholder="请输入权限名称（最多50个字符）" :maxlength="50" show-word-limit
            clearable></el-input>
        </my-form-item>
        <my-form-item prop="code" label="权限代码" required>
          <el-input v-model="permissionsFormData.code" placeholder="请输入权限代码（最多50个字符）" :maxlength="50" show-word-limit
            clearable></el-input>
        </my-form-item>
        <my-form-item>
          <el-button type="primary" @click="handleSubmitPermissions" class="submit-btn">提交</el-button>
          <el-button @click="closePermissionsDrawer" class="cancel-btn">取消</el-button>
        </my-form-item>
      </my-form>
    </drawer>
  </div>
...
// 引入api
import { get_all_permissions, create_permission, update_permission, delete_permission } from '@/api/user'
...
// 获取用户权限列表
// const fetchUserpermission = async (keyword?: string) => {
//     const res: any = await list_permission(keyword)

//     PermissionsList.value = res || []
// }

// 获取全部权限信息
const fetchPermissions = async (params?: { search?: string }) => {
  const res: any = await get_all_permissions(params)
  console.log(res);
...
// 获取权限列表
const fetchPermissionsList = async (resetPage = false) => {
  if (resetPage) {
  ...
  try {
    ...
    toast({
      message: err?.response?.data?.message || '获取权限列表失败',
      type: 'error'
    })
...
const handleSortChange = ({ column, prop, order }: any) => {
  ...
  // 可以重新调用 fetchPermissionsList 并传递排序参数
}
...
// 是否打开权限抽屉
const isOpenPermissionsDrawer = ref(false)

// 当前是新增还是编辑
const isAddMode = ref(true)

// 当前编辑的权限ID
const currentPermissionsId = ref<number | null>(null)

// 抽屉标题
const drawerTitle = computed(() => {
  return isAddMode.value ? '新增权限' : '编辑权限'
})
...
// 权限表单验证规则
const permissionsFormRules: FormRuleMap = reactive({
  name: [
    { required: true, message: '请输入权限名称' },
    { max: 50, message: '权限名称最多50个字符' }
  ],
  code: [
    { required: true, message: '请输入权限代码名称' },
    { max: 50, message: '权限代码最多50个字符' }
  ],
})

// 打开新增权限抽屉
const openAddPermissionsDrawer = () => {
  isAddMode.value = true
  currentPermissionsId.value = null
  resetPermissionsForm()
  isOpenPermissionsDrawer.value = true
}

// 打开编辑权限抽屉
const openEditPermissionsDrawer = (Permissions: any) => {
  isAddMode.value = false
  currentPermissionsId.value = Permissions.id
  permissionsFormData.name = Permissions.name || ''
  permissionsFormData.code = Permissions.code || ''
...
const closePermissionsDrawer = () => {
  isOpenPermissionsDrawer.value = false
  resetPermissionsForm()
}

// 重置权限表单
const resetPermissionsForm = () => {
  permissionsFormData.name = ''
  permissionsFormData.code = ''
}
...
const handleSubmitPermissions = async () => {
  ...
  if (permissionsFormRef.value) {
    const isValid = await permissionsFormRef.value.validate()
    if (!isValid) {
      return
    }
  }
  ...
  try {
    if (isAddMode.value) {
      ...
      await create_permission({
        name: permissionsFormData.name.trim(),
        code: permissionsFormData.code.trim()
      })
      toast({
        message: '权限创建成功',
        type: 'success'
      ...
      if (!currentPermissionsId.value) {
        console.log('未获取到权限id');

        return
      }
      console.log({
        name: permissionsFormData.name,
        permissions: permissionsFormData.code
      });


      // TODO: 调用编辑权限 API
      await update_permission(currentPermissionsId.value, {
        name: permissionsFormData.name.trim(),
        code: permissionsFormData.code.trim()
      })
      toast({
...
// 处理删除权限
const handleDeletePermissions = async (Permissions: any) => {
  const isConfirm = await useConfirm({
    title: '确认删除',
    message: `确定要删除权限 "${Permissions.name}" 吗？此操作不可恢复。`,
    confirmText: '删除',
    cancelText: '取消'
  })
  ...
  try {
    // TODO: 调用删除权限 API
    await delete_permission(Permissions.id)
    toast({
      message: '权限删除成功',
      type: 'success'
    })
    ...
    toast({
      message: err?.response?.data?.message || '删除权限失败',
      type: 'error'
    })
  }
}
...
.user-manage-permission {
  ...
  // 顶部区域
  .user-manage-permission__top-region {
    display: flex;
    align-items: center;
  ...
  // 权限列表表格内容区域
  .user-manage-permission__main-table {
    flex: 1;
    min-height: 0;
    overflow: auto;
    margin-bottom: 16px;
  ...
  // 分页器
  .user-manage-permission__pagination {
    display: flex;
